...
async function handleButton(interaction) {
  ...
  // ── Pagination ────────────────────────────────────────────────────────────
  if (id.startsWith('ga_pg:')) {
    const [, pgGaId, rawPage] = id.split(':');
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({ text: `Page ${page + 1}/${pages.length} · ${totalUnique} participants · ${totalEntries} entries · ${FOOTER_BRAND}` });
  ...
  // ── Bonus Roles ───────────────────────────────────────────────────────────
  if (id.startsWith('ga_roles:')) {
    const ga = await queryOne('SELECT * FROM giveaways WHERE id = $1', [gaId]);
    if (!ga) return interaction.reply({ content: '❌ Giveaway not found.', ephemeral: true });

    const extraEntries = ga.extra_entries ? JSON.parse(ga.extra_entries) : [];
    if (!extraEntries.length)
      return interaction.reply({ content: 'ℹ️ This giveaway has no bonus roles.', ephemeral: true });

    const embed = new EmbedBuilder()
      .setColor(PRIMARY)
      .setTitle('<:gift:1491499820379275366>  Bonus Roles')
      .setDescription(extraEntries.map(ed => `<@&${ed.role_id}> — **+${ed.count}** entr${ed.count !== 1 ? 'ies' : 'y'}`).join('\n'))
      .setFooter({ text: FOOTER_BRAND });
...
const exps = [giveawayCmd, endGiveawayCmd];
exps.handleButton = handleButton;
module.exports = exps;
